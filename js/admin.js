...
// Hàm kiểm tra chuỗi không chứa ký tự đặc biệt (chỉ chữ và số)
function hasNoSpecialChars(str) {
    if (!str) return false;
    return /^[a-zA-Z0-9\sÀ-ỹ]+$/.test(str);
}

// Hàm validation đầy đủ
function validateSanPham(name, company, img, price, star, rateCount, promoName, promoValue, 
                        screen, os, camara, camaraFront, cpu, ram, rom, microUSB, battery, isAdding) {
    
    // 1. Tên sản phẩm: text, 3-50 ký tự
    if (!name || name.trim() === '') {
        alert('Vui lòng nhập tên sản phẩm');
        return false;
    }
    var nameTrim = name.trim();
    if (nameTrim.length < 3 || nameTrim.length > 50) {
        alert('Tên sản phẩm phải từ 3 đến 50 ký tự');
        return false;
    }

    // 2. Hãng: chọn 1 trong 15 hãng điện thoại
    var validCompanies = ["Apple", "Samsung", "Oppo", "Nokia", "Huawei", "Xiaomi", "Realme", 
                          "Vivo", "Philips", "Mobell", "Mobiistar", "Itel", "Coolpad", "HTC", "Motorola"];
    if (!company || validCompanies.indexOf(company) === -1) {
        alert('Vui lòng chọn một trong 15 hãng điện thoại');
        return false;
    }

    // 3. Hình:
    // - Khi THÊM: phải chọn 1 ảnh có sẵn trên web (dropdown) => img phải khác rỗng
    // - Khi SỬA: cho phép ảnh hiện có hoặc ảnh mới (upload)
    if (isAdding) {
        if (!img || img.trim() === '') {
        ...
        if ((!previewSrc || previewSrc === '') && (!img || img === '')) {
            alert('Vui lòng chọn ảnh từ máy');
            return false;
        }
    }

    // 4. Giá tiền: dạng số, không âm
    if (!price || price.trim() === '') {
    ...
    if (isNaN(priceNum) || priceNum < 0) {
        alert('Giá tiền phải là số và không được âm');
        return false;
    }

    // 5. Số sao: số nguyên 0-5
    if (star === '' || star === null || star === undefined) {
        alert('Vui lòng nhập số sao');
        return false;
    }
    var starNum = parseInt(star);
    if (isNaN(starNum) || starNum < 0 || starNum > 5) {
        alert('Số sao phải là số nguyên từ 0 đến 5');
        return false;
    }

    // 6. Đánh giá: dạng số, không âm
    if (!rateCount || rateCount.trim() === '') {
        alert('Vui lòng nhập đánh giá');
        return false;
    }
    var rateCountNum = parseFloat(rateCount);
    if (isNaN(rateCountNum) || rateCountNum < 0) {
        alert('Đánh giá phải là số và không được âm');
        return false;
    }

    // 7. Khuyến mãi: chọn 1 trong 5 loại
    var validPromos = ["", "tragop", "giamgia", "giareonline", "moiramat"];
    if (validPromos.indexOf(promoName) === -1) {
        alert('Vui lòng chọn một trong 5 loại khuyến mãi');
        return false;
    }

    // 8. Giá trị khuyến mãi: chuỗi ký tự, 1-10 ký tự, không số âm, không ký tự đặc biệt
    if (promoName && promoName !== '') {
        if (!promoValue || promoValue.trim() === '') {
            alert('Vui lòng nhập giá trị khuyến mãi');
            return false;
        }
        var promoValueTrim = promoValue.trim();
        if (promoValueTrim.length < 1 || promoValueTrim.length > 10) {
            alert('Giá trị khuyến mãi phải từ 1 đến 10 ký tự');
            return false;
        }
        // Kiểm tra số âm
        var promoNum = parseFloat(promoValueTrim);
        if (!isNaN(promoNum) && promoNum < 0) {
            alert('Giá trị khuyến mãi không được là số âm');
            return false;
        }
        // Kiểm tra ký tự đặc biệt (chỉ cho phép chữ, số, dấu chấm, dấu phẩy, dấu %)
        if (!/^[a-zA-Z0-9\sÀ-ỹ.,%]+$/.test(promoValueTrim)) {
            alert('Giá trị khuyến mãi không được chứa ký tự đặc biệt');
            return false;
        }
    }

    // 9. Màn hình: chuỗi ký tự, 3-70 ký tự
    if (!screen || screen.trim() === '') {
        alert('Vui lòng nhập màn hình');
        return false;
    }
    var screenTrim = screen.trim();
    if (screenTrim.length < 3 || screenTrim.length > 70) {
        alert('Màn hình phải từ 3 đến 70 ký tự');
        return false;
    }

    // 10. Hệ điều hành: chuỗi ký tự, 3-70 ký tự
    if (!os || os.trim() === '') {
        alert('Vui lòng nhập hệ điều hành');
        return false;
    }
    var osTrim = os.trim();
    if (osTrim.length < 3 || osTrim.length > 70) {
        alert('Hệ điều hành phải từ 3 đến 70 ký tự');
        return false;
    }

    // 11. Camera sau: chuỗi số, 1-10 ký tự, không số âm, không chữ
    if (!camara || camara.trim() === '') {
        alert('Vui lòng nhập camera sau');
        return false;
    }
    var camaraTrim = camara.trim();
    if (camaraTrim.length < 1 || camaraTrim.length > 10) {
        alert('Camera sau phải từ 1 đến 10 ký tự');
        return false;
    }
    if (!isNumericString(camaraTrim)) {
        alert('Camera sau phải là chuỗi số');
        return false;
    }
    var camaraNum = parseInt(camaraTrim);
    if (camaraNum < 0) {
        alert('Camera sau không được là số âm');
        return false;
    }

    // 12. Camera trước: chuỗi số, 1-10 ký tự, không số âm, không chữ, không ký tự đặc biệt
    if (!camaraFront || camaraFront.trim() === '') {
        alert('Vui lòng nhập camera trước');
        return false;
    }
    var camaraFrontTrim = camaraFront.trim();
    if (camaraFrontTrim.length < 1 || camaraFrontTrim.length > 10) {
        alert('Camera trước phải từ 1 đến 10 ký tự');
        return false;
    }
    if (!isNumericString(camaraFrontTrim)) {
        alert('Camera trước phải là chuỗi số');
        return false;
    }
    var camaraFrontNum = parseInt(camaraFrontTrim);
    if (camaraFrontNum < 0) {
        alert('Camera trước không được là số âm');
        return false;
    }

    // 13. CPU: chuỗi ký tự, 5-50 ký tự
    if (!cpu || cpu.trim() === '') {
        alert('Vui lòng nhập CPU');
        return false;
    }
    var cpuTrim = cpu.trim();
    if (cpuTrim.length < 5 || cpuTrim.length > 50) {
        alert('Tên CPU không hợp lệ. CPU phải từ 5 đến 50 ký tự');
        return false;
    }

    // 14. RAM: chuỗi số, 1-10 ký tự (đơn vị GB), không số âm, không ký tự đặc biệt, không chữ
    if (!ram || ram.trim() === '') {
        alert('Vui lòng nhập RAM');
        return false;
    }
    var ramTrim = ram.trim();
    if (ramTrim.length < 1 || ramTrim.length > 10) {
        alert('RAM phải từ 1 đến 10 ký tự');
        return false;
    }
    // RAM có thể có "GB" ở cuối, nên kiểm tra số trước "GB"
    var ramMatch = ramTrim.match(/^(\d+)\s*GB?$/i);
    if (!ramMatch && !isNumericString(ramTrim)) {
        alert('RAM phải là số (đơn vị GB)');
        return false;
    }
    var ramNum = ramMatch ? parseInt(ramMatch[1]) : parseInt(ramTrim);
    if (ramNum < 0) {
        alert('RAM không được là số âm');
        return false;
    }

    // 15. Bộ nhớ trong: chuỗi ký tự, 2-10 ký tự (đơn vị GB), không số âm, không ký tự đặc biệt
    if (!rom || rom.trim() === '') {
        alert('Vui lòng nhập bộ nhớ trong');
        return false;
    }
    var romTrim = rom.trim();
    if (romTrim.length < 2 || romTrim.length > 10) {
        alert('Bộ nhớ trong phải từ 2 đến 10 ký tự');
        return false;
    }
    // Bộ nhớ trong có thể có "GB" ở cuối
    var romMatch = romTrim.match(/^(\d+)\s*GB?$/i);
    if (!romMatch && !isNumericString(romTrim)) {
        alert('Bộ nhớ trong phải là số');
        return false;
    }
    var romNum = romMatch ? parseInt(romMatch[1]) : parseInt(romTrim);
    if (romNum < 0) {
        alert('Bộ nhớ trong không được là số âm');
        return false;
    }
    // Kiểm tra ký tự đặc biệt (cho phép số và GB)
    if (!/^[0-9\sGBgb]+$/.test(romTrim)) {
        alert('Bộ nhớ trong không được chứa ký tự đặc biệt');
        return false;
    }

    // 16. Thẻ nhớ: chuỗi số, 1-5 ký tự (đơn vị GB), không số âm, không ký tự đặc biệt, không chữ
    if (!microUSB || microUSB.trim() === '') {
        alert('Vui lòng nhập thông số thẻ nhớ');
        return false;
    }
    var microUSBTrim = microUSB.trim();
    if (microUSBTrim.length < 1 || microUSBTrim.length > 5) {
        alert('Vui lòng nhập thông số thẻ nhớ hợp lệ. Thẻ nhớ phải từ 1 đến 5 ký tự');
        return false;
    }
    // Thẻ nhớ có thể có "GB" ở cuối
    var microUSBMatch = microUSBTrim.match(/^(\d+)\s*GB?$/i);
    if (!microUSBMatch && !isNumericString(microUSBTrim)) {
        alert('Thẻ nhớ phải là số (đơn vị GB)');
        return false;
    }
    var microUSBNum = microUSBMatch ? parseInt(microUSBMatch[1]) : parseInt(microUSBTrim);
    if (microUSBNum < 0) {
        alert('Thẻ nhớ không được là số âm');
        return false;
    }

    // 17. Dung lượng Pin: chuỗi số, 4-10 ký tự, không số âm, không ký tự đặc biệt, không chữ
    if (!battery || battery.trim() === '') {
        alert('Vui lòng nhập dung lượng pin');
        return false;
    }
    var batteryTrim = battery.trim();
    if (batteryTrim.length < 4 || batteryTrim.length > 10) {
        alert('Vui lòng nhập dung lượng pin hợp lệ. Dung lượng pin phải từ 4 đến 10 ký tự');
        return false;
    }
    if (!isNumericString(batteryTrim)) {
        alert('Dung lượng pin phải là số');
        return false;
    }
    var batteryNum = parseInt(batteryTrim);
    if (batteryNum < 0) {
        alert('Dung lượng pin không được là số âm');
        return false;
    }
...
function layThongTinSanPhamTuTable(id) {
    ...
    var isAdding = (id === 'khungThemSanPham');

    // Form THÊM đã xóa dòng "Hình" nên index khác form SỬA
    var img, price, star, rateCount, promoName, promoValue;
    var screen, os, camara, camaraFront, cpu, ram, rom, microUSB, battery;

    if (isAdding) {
        // Ở form THÊM: Hình là dropdown (không upload)
        img = tr[4].getElementsByTagName('td')[1].getElementsByTagName('select')[0].value;
        price = tr[5].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
        star = tr[6].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
        rateCount = tr[7].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
        promoName = tr[8].getElementsByTagName('td')[1].getElementsByTagName('select')[0].value;
        promoValue = tr[9].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;

        screen = tr[11].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
        os = tr[12].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
        camara = tr[13].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
        camaraFront = tr[14].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
        cpu = tr[15].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
        ram = tr[16].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
        rom = tr[17].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
        microUSB = tr[18].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
        battery = tr[19].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
    } else {
        img = tr[4].getElementsByTagName('td')[1].getElementsByTagName('img')[0].src;
        price = tr[5].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
        star = tr[6].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
        rateCount = tr[7].getElementsByTagName('td')[1].getElementsByTagName('input')[0].value;
        promoName = tr[8].getElementsByTagName('td')[1].getElementsByTagName('select')[0].value;
    ...
    // Validation đầy đủ
    if (!validateSanPham(name, company, img, price, star, rateCount, promoName, promoValue,
                         screen, os, camara, camaraFront, cpu, ram, rom, microUSB, battery, isAdding)) {
        return false;
    }

    try {
        // Sử dụng previewSrc nếu có (ảnh mới), nếu không dùng img
        // - THÊM: img là ảnh chọn sẵn
        // - SỬA: img là ảnh hiện có
        var finalImg = (previewSrc && previewSrc !== '') ? previewSrc : img;
        ...
        return {
            ...
            "promo": {
                "name": promoName,
                "value": promoValue.trim()
            },
            "detail": {
                "screen": screen.trim(),
                "os": os.trim(),
                "camara": camara.trim(),
                "camaraFront": camaraFront.trim(),
                "cpu": cpu.trim(),
                "ram": ram.trim(),
                "rom": rom.trim(),
                "microUSB": microUSB.trim(),
...
// Chọn ảnh có sẵn khi THÊM sản phẩm
function chonAnhSanPhamThem(sel) {
    var v = sel.value || '';
    var img = document.getElementById('anhDaiDienSanPhamThem');
    if (img) img.src = v;
}
...
function addKhungSuaSanPham(masp) {
    // Reset previewSrc khi mở form sửa
    previewSrc = '';
